...
2. **Publish the combined tag the catalog lists.** Tag format is `<semver>-ocp<major.minor>` (not `ocp4.22-1.0.1`, not bare `:1.1.0`, not bare `:4.22`). **Always publish both** `-ocp4.21` and `-ocp4.22`. Never list a catalog row for (version, OpenShift minor) unless that exact tag exists and is public. That is what caused Add-success / Open-404 (`:1.1.0` cataloged, only `:4.22` published). Bare `:4.22` / `:1.1.0` may remain as extra aliases.
...
7. **`spec.href` matches plugin routes.** Tile **Open** uses `spec.href`. It must be a path registered in the extension `console-extensions.json`. Current: `oct-baremetal` → `/baremetal/nodes`; `oct-network-bond` → `/community-tools/network/bond`; `oct-banner` → `/community-tools/management/banner`; `oct-windows-builder` → `/community-tools/compute/windows-builder` unless those routes changed.
...
**Always publish both OpenShift minors.** Every published image (plugin, sidecars, storefront, catalog-service when rebuilt) gets **both** tags: `<semver>-ocp4.21` and `<semver>-ocp4.22`. Same bits → two tags on one digest. Never catalog a (version, minor) unless that exact public tag exists. Catalog `versions[]` lists both minors when the tags exist. Prefer one `versions[]` row per (semver, OCP minor) so `image` is the exact combined tag.
...
Keep `imagePullPolicy` IfNotPresent. Do not make Always the product default. **Any storefront UI change** (picker, icons, copy) bumps storefront semver and `install.yaml` to a **new combined tag** (e.g. `1.2.0-ocp4.22`), and publish the matching `-ocp4.21` tag too. Never ship plugin JS by retagging the only install tag. Customers who apply after a catalog-only commit see new tiles, not new storefront chrome.
