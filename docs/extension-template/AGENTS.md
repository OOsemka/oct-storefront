...
- **Two version axes in the catalog:** git tag `v1.2.0` (semver) and optional `ocp-X.Y` branch for PF/API. Image tags **always** `<semver>-ocp<major.minor>` (e.g. `oct-<name>:1.2.0-ocp4.22`). **Always publish both** `-ocp4.21` and `-ocp4.22` (same digest if bits match). Never catalog `:1.2.0` or `:4.22` as the install image unless that exact combined tag exists and is public.
