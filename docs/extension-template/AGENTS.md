...
- Catalog PR: `oct-storefront` `catalog/community.yaml` with `spec.versions[]` (`version`, `channel`, `openshift`, `image`). **Images must be public** (public Quay); private images break tile Add on other clusters.
